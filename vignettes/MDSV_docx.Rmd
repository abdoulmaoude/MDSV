...
vars <- c("omega","a","b","sigma","v0","shape")
tmp <- c(0.52,0.85, 2.77,sqrt(var(sp500[,1])),0.72,2.10)
if(LEVIER) {
  vars <- c(vars,"l","theta")
  tmp <- c(tmp,1.5,0.87568)
}
...
out       <- MDSVfit(K = K, N = N, data = sp500, ModelType = 1, LEVIER = FALSE, start.pars=tmp)
# Summary
summary(out)
# Plot
plot(out,c("dis"))
...
tmp <- c(0.52,0.85, 2.77,sqrt(var(sp500[,1])),0.72,-1.5,	0.72,	-0.09,	0.04,	2.10)
if(LEVIER) {
  vars <- c(vars,"l","theta")
  tmp <- c(tmp,1.5,0.87568)
}
...
out       <- MDSVfit(K = K, N = N, data = sp500, ModelType = 2, LEVIER = LEVIER, start.pars=tmp)
# Summary
summary(out)
# Plot
plot(out,c("dis","nic"))
...
names(tmp) <- vars
out_fit   <- MDSVfit(K = K, N = N, data = sp500, ModelType = 0, LEVIER = FALSE, start.pars = tmp)
...
names(tmp) <- vars
out_fit   <- MDSVfit(K = K, N = N, data = sp500, ModelType = 1, LEVIER = FALSE, start.pars = tmp)
out       <- MDSVboot(fit = out_fit, n.ahead = 100, n.bootpred = 10000, rseed = 349)
# Summary
summary(out)
...
out_fit   <- MDSVfit(K = K, N = N, data = sp500, ModelType = 2, LEVIER = LEVIER, start.pars = tmp)
out       <- MDSVboot(fit = out_fit, n.ahead = 100, n.bootpred = 10000, rseed = 349)
# Summary
summary(out)
